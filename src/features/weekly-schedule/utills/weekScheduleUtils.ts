...
const isTodayCell = (weekDate: Dayjs, dayIndex: number, currentTime: Dayjs): boolean => {
    return weekDate.add(dayIndex, "day").isSame(currentTime, "day");
}
...
export const isCurrentTimeInCell = (weekDate: Dayjs, dayIndex: number, hour: number, currentTime: Dayjs): boolean => {
    ...
    return isTodayCell(weekDate, dayIndex, currentTime) && currentHour === hour;
};

// 현재 시간 인디케이터 위치 계산
export const getCurrentTimePosition = (weekDate: Dayjs, dayIndex: number, currentTime: Dayjs): number | null => {
   if (!isTodayCell(weekDate, dayIndex, currentTime)) return null;
